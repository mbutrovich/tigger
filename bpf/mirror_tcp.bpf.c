...
int32_t _mp_mirror_tcp(struct __sk_buff *const skb) {
  // bpf_printk("");
  ...
  if (bpf_ntohl(skb->remote_port) == 5432) {
    ...
    // bpf_printk("mirror traffic from %u", server_socket_key);
    ...
    if (txn_status == 'I') {
      // The replica is Idle. See if we're ready to proceed.

      const uint32_t primary_socket_key = skb->local_port;

      MirrorResponses *responses = bpf_map_lookup_elem(&mirror_responders, &primary_socket_key);
      ...
      if (responses->wait_) {
        // Primary has already responded. We're okay to proceed.
        responses->wait_ = false;
      } else {
        // Mirror is the first to respond. Start blocking the client.
